...
export default function TodoList() {
  ...
  useEffect(() => {
    ...
    console.log("Item: " + currentTask.taskName);
    if (currentTask.taskName) {
      if (currentTask.taskName.includes("delete")) {
        deleteTaskVoice(currentTask.taskName.replace(/delete/g, "").trim());
      } else if (currentTask.taskName.includes("complete") || currentTask.taskName.includes("mark")) {
        ...
        if (!IsProductFound) {
          Alert.alert(
            "Sorry",
            `The product is not on the list: ${currentTask.taskName}`
          );
        }
        handleClear();
      } else {
  ...
  // REFRESHES WINDOW TO SHOW LATEST UPDATES
  useEffect(() => {
  ...
  const unCheckTaskVoice = async (taskNameToDelete: string) => {
    ...
    if (index !== -1) {
      try {
        ...
        Alert.alert("Error", "Failed to update product");
        console.log("Error", "Failed to update product: " + taskNameToDelete);
      }
    }
  };
